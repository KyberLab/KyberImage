...
IMAGE_EXPORT_ENV			:= \
	IMAGE_EXPORT_ENV_ENABLE=1

IMAGE_EXPORT_ENV			+= \
	$(if $(BUILD_PLATFORM),BUILD_PLATFORM=$(BUILD_PLATFORM)) \
	$(if $(BUILD_BOARD),BUILD_BOARD=$(BUILD_BOARD)) \
...
IMAGE_EXPORT_ENV				+= \
	...
	$(if $(BUILD_SUBVERSION),BUILD_SUBVERSION=$(BUILD_SUBVERSION)) \
	$(if $(BUILD_REVISION),BUILD_REVISION=$(BUILD_REVISION)) \
	$(if $(BUILD_SCMVERSION),BUILD_SCMVERSION=$(BUILD_SCMVERSION))

IMAGE_EXPORT_ENV				+= \
	$(if $(WORKSPACE_ROOT_PATH),WORKSPACE_ROOT_PATH=$(WORKSPACE_ROOT_PATH)) \
	$(if $(BENCH_ROOT_PATH),BENCH_ROOT_PATH=$(BENCH_ROOT_PATH)) \
	$(if $(RULES_ROOT_PATH),RULES_ROOT_PATH=$(RULES_ROOT_PATH)) \
	$(if $(BUILD_ROOT_PATH),BUILD_ROOT_PATH=$(BUILD_ROOT_PATH)) \
	$(if $(OUTPUT_ROOT_PATH),OUTPUT_ROOT_PATH=$(OUTPUT_ROOT_PATH)) \
	$(if $(IMAGE_ROOT_PATH),IMAGE_ROOT_PATH=$(IMAGE_ROOT_PATH))
